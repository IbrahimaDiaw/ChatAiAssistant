...
namespace ChatAI_Assistant.Server.Services.AI
{
    public class OpenAIService : IAIService
    {
        ...
        private readonly OpenAISettings _config;
        private readonly ILogger<OpenAIService> _logger;

        public AIProvider ProviderType => AIProvider.OpenAI;

        public OpenAIService(HttpClient httpClient, OpenAISettings config, ILogger<OpenAIService> logger)
        {
        ...
        public async Task<AIResponse> GenerateResponseWithSettingsAsync(string message, OpenAISettings settings, string? context = null)
        {
            try
            {
                ...
                _logger.LogDebug("Sending request to OpenAI API with {MessageCount} messages", messages.Count);

                var response = await SendRequestWithRetryAsync(requestPayload);

                if (response == null)
                {
                    return CreateErrorResponse("Failed to get response from OpenAI API");
                }
            ...
            catch (TaskCanceledException ex)
            {
                ...
                return CreateErrorResponse("Request timeout - OpenAI API took too long to respond");
            }
            catch (Exception ex)
            {
        ...
        private List<OpenAIMessage> BuildMessages(string message, string? context, string? systemPrompt)
        {
            ...
            if (!string.IsNullOrWhiteSpace(context))
            {
                ...
                foreach (var line in contextLines.TakeLast(5)) // Limiter le contexte
                {
                    if (line.Contains(": "))
                    {
                        ...
                        messages.Add(new OpenAIMessage { Role = role, Content = parts[1] });
                    }
                }
            }
        ...
        private object CreateRequestPayload(List<OpenAIMessage> messages, OpenAISettings settings)
        {
            return new
            {
        ...
        private async Task<OpenAIApiResponse?> SendRequestWithRetryAsync(object requestPayload)
        {
            ...
            for (int attempt = 1; attempt <= 3; attempt++)
            {
                try
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        var apiResponse = JsonSerializer.Deserialize<OpenAIApiResponse>(responseContent, new JsonSerializerOptions
                        {
                            PropertyNamingPolicy = JsonNamingPolicy.CamelCase
                        });
                    ...
                    else
                    {
                        ...
                        _logger.LogWarning("OpenAI API returned {StatusCode}: {Error}", response.StatusCode, errorContent);
        ...
        private AIResponse CreateSuccessResponse(OpenAIApiResponse response, OpenAISettings settings)
        {
